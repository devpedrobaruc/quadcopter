...
void FlightController::Serial_input_handler(void)
{
  ...
  si_measured_time = TIM3->CCR1 - si_measured_time_start;
  if (si_measured_time < 0)
    si_measured_time += 0xFFFF;

  si_last_input_change = millis();
  if (si_last_input_change - si_last_input_change_previous > 50)
  {
    si_time_array_counter = 1;
    si_measured_time_start = TIM3->CCR1;
  }
  else
  {
    si_time_array[si_time_array_counter] = si_measured_time;
    if (si_time_array_counter < 180)
      si_time_array_counter++;
  }
