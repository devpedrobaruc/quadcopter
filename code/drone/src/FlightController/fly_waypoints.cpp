...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void FlightController::fly_waypoints(void)
{
  waypoint_monitor = 0;
  ...
  if (fly_to_new_waypoint == 1)
  {
    ...
    if (fly_to_new_waypoint_step == 0)
    {
      ...
      if (fly_to_waypoint_lat_factor == 1 || fly_to_waypoint_lon_factor == 1)
        fly_to_new_waypoint_step = 1;
    ...
    {
      waypoint_monitor = 6;
      if (wp_lat_gps == l_lat_waypoint && wp_lon_gps == l_lon_waypoint)
        fly_to_new_waypoint = 0;

      if (abs(wp_lat_gps - l_lat_waypoint) < 160 && abs(wp_lon_gps - l_lon_waypoint) < 160 && fly_to_waypoint_move_factor > 0.05)
        fly_to_waypoint_move_factor -= 0.00015;
      else if (fly_to_waypoint_move_factor < 0.20)
        fly_to_waypoint_move_factor += 0.0001;

      if (wp_lat_gps != l_lat_waypoint)
      {
        if (wp_lat_gps > l_lat_waypoint)
          l_lat_gps_float_adjust += fly_to_waypoint_move_factor * fly_to_waypoint_lat_factor;
        if (wp_lat_gps < l_lat_waypoint)
          l_lat_gps_float_adjust -= fly_to_waypoint_move_factor * fly_to_waypoint_lat_factor;
      }
      ...
      {
        if (wp_lon_gps > l_lon_waypoint)
          l_lon_gps_float_adjust += fly_to_waypoint_move_factor * fly_to_waypoint_lon_factor;
        if (wp_lon_gps < l_lon_waypoint)
          l_lon_gps_float_adjust -= fly_to_waypoint_move_factor * fly_to_waypoint_lon_factor;
      }
    }
