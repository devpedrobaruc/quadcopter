...
void FlightController::read_barometer(void)
{
  barometer_counter++;
  ...
  { // When the barometer_counter variable is 1.
    ...
    { // And the temperature counter is 0.
      ...
      HWire.requestFrom(MS5611_address, 3U);   // Poll 3 data bytes from the MS5611.
  ...
  { // When the barometer counter is 3
    ...
    if (parachute_rotating_mem_location == 30)
      parachute_rotating_mem_location = 0; // Start at 0 when the memory location 20 is reached.
    ...
    {                             // If the altitude hold mode is not set and the PID altitude setpoint is still set.
